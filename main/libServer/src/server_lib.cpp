...
namespace thunderchat
{
	...
	bool ThunderChatServer::Accept_Client() noexcept
	{
		...
		else if (selectReady > 0)
		{
			...
			if (msg != nlohmann::detail::value_t::discarded)
			{
				...
				std::for_each(m_onConnectCallbacks.begin(), m_onConnectCallbacks.end(), [](connectCallbackType callback) { // ICI
					//callback(Client.);
				});
	...
	{
		std::for_each(m_onDisconnectCallbacks.begin(), m_onDisconnectCallbacks.end(), [](disconnectCallbackType callback) { // ICI
					//callback(Client.);
		});
